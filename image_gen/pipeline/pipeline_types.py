...
class ControlNetConfig:
    ...
    def __post_init__(self):
        ...
        if not self.image_path.is_file():
            raise FileNotFoundError("image path must be a file path")
...
class LoraConfig:
    ...
    def __post_init__(self):
        ...
        if not self.lora_path.is_file():
            raise FileNotFoundError("lora_path must be a file path")
...
class PipelineConfigs:
    ...
    vae: Optional[Union[Literal["anime", "realistic", "semi-realistic", "default"], str, Path]] = (
    ...
    # --- 9. CONTROLNET ---
    c_net: List[ControlNetConfig] = field(
        default_factory=list
    )  # Mostly used for getting the particular noise from the image
    ...
    def __post_init__(self):
        ...
        if not self.base_model.is_file():
            raise FileNotFoundError("base_model must be a file path")

        if self.steps <= 0:
            raise ValueError("Steps needs to be more than 0.")
        if self.steps >= 45:
            raise ValueError("Steps needs to be less than 45.")

        if self.cfg <= 0:
            raise ValueError("CFG needed to be more than 0")
        if self.cfg >= 15:
            raise ValueError("CFG needed to be less than 15")
